...
from gridos.models.common import ControlCommand, ControlMode, DERStatus, DERTelemetry, DERType, DeviceInfo, TelemetryBatch
...
class TestControlCommand:
    """Validate the reduced control command model."""

    def test_create_power_setpoint_command(self) -> None:
        ...
        assert command.mode == ControlMode.POWER_SETPOINT
        assert command.setpoint_kw == 25.0
        assert command.command_id is not None

    def test_create_soc_target_command(self) -> None:
        command = ControlCommand(
            device_id="battery-001",
            mode=ControlMode.SOC_TARGET,
            soc_target_percent=80.0,
        )

        assert command.mode == ControlMode.SOC_TARGET
        assert command.soc_target_percent == 80.0

    def test_require_mode_specific_fields(self) -> None:
        with pytest.raises(ValidationError):
            ControlCommand(device_id="battery-001", mode=ControlMode.POWER_SETPOINT)
